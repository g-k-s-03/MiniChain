...
def test_tampered_amount_fails_verification(alice, bob):
    ...
    tx = Transaction(alice_pk, bob_pk, 10, nonce=0)
    tx.sign(alice_sk)
    object.__setattr__(tx, "amount", 9999)  # tamper
...
def test_tampered_receiver_fails_verification(alice, bob):
    ...
    object.__setattr__(tx, "receiver", attacker_sk.verify_key.encode(encoder=HexEncoder).decode())  # tamper
...
def test_tampered_nonce_fails_verification(alice, bob):
    ...
    tx = Transaction(alice_pk, bob_pk, 10, nonce=0)
    tx.sign(alice_sk)
    object.__setattr__(tx, "nonce", 99)  # tamper
...
def test_forged_sender_field_fails_verification(alice, bob):
    ...
    tx = Transaction(alice_pk, bob_pk, 10, nonce=0)
    tx.sign(alice_sk)
    object.__setattr__(tx, "sender", bob_pk)  # forge sender
